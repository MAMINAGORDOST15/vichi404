...
app.post('/api/contact', rateLimit(3, 60000), async (req, res) => {
  ...
  await sendTelegram(`NEW MESSAGE!\n\nName: ${name.trim()}\nContact: ${contact.trim()}\nMessage: ${message.trim()}\n\nTime: ${new Date().toLocaleString('ru-RU',{timeZone:'Asia/Jerusalem'})}`);
  res.json({ ok: true });
});
...
app.get('/api/admin/data', (req, res) => {
  const auth = req.query.pw || (req.headers.authorization || '').replace('Bearer ', '');
...
// ── ADMIN PAGE (single page, no redirect) ──
app.get('/admin', (req, res) => {
  res.setHeader('Content-Type', 'text/html');
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin - VICHI404</title>
<style>
...
h1{font-size:24px;letter-spacing:6px;margin-bottom:8px}
.sub{color:rgba(255,255,255,0.3);font-size:11px;letter-spacing:2px;margin-bottom:32px}
...
.btn{width:100%;background:#fff;color:#0a0a0a;border:none;padding:14px;font-size:11px;letter-spacing:2.5px;text-transform:uppercase;cursor:pointer;border-radius:2px;font-weight:600}
.err{color:#ff6b6b;font-size:12px;margin-top:12px;display:none}
...
.logo{font-size:16px;letter-spacing:4px;font-weight:600}
.logout{background:none;border:0.5px solid rgba(255,255,255,0.15);color:rgba(255,255,255,0.4);padding:6px 14px;border-radius:2px;cursor:pointer;font-size:11px}
main{padding:32px}
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:2px;margin-bottom:2px}
...
.stat-label{font-size:9px;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.3);margin-top:8px}
...
.bf{height:100%;background:rgba(255,255,255,0.7);border-radius:2px}
.bc{font-size:11px;color:rgba(255,255,255,0.3);width:28px;text-align:right}
.empty{text-align:center;padding:48px;color:rgba(255,255,255,0.15);font-size:13px;letter-spacing:1px}
@media(max-width:600px){.charts{grid-template-columns:1fr}.order-top{grid-template-columns:1fr auto}}
</style></head>
<body>
<div id="login">
  <div class="box">
...
<div id="dashboard">
  <nav><div class="logo">VICHI404 · ADMIN</div><button class="logout" onclick="logout()">Logout</button></nav>
  <main id="main"><div class="empty">Loading...</div></main>
</div>
<script>
let pw = localStorage.getItem('adminpw') || '';
if(pw) tryLoad();
...
function tryLoad(){
  fetch('/api/admin/data?pw='+encodeURIComponent(pw))
    .then(r=>{
      if(r.status===401){ document.getElementById('err').style.display='block'; pw=''; return null; }
...
function logout(){ localStorage.removeItem('adminpw'); pw=''; document.getElementById('dashboard').style.display='none'; document.getElementById('login').style.display='flex'; }

function changeStatus(id,status){
  fetch('/api/admin/order/'+id+'/status?pw='+encodeURIComponent(pw),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}).then(()=>load());
}
...
function render({orders,stats}){
  ...
  const maxL=Math.max(...Object.values(stats.langStats||{0:1}),1);
