...
function login(){
  ...
  fetch('/admin/data',{headers:{'Authorization':'Bearer '+pw}})
    .then(r=>{if(r.ok){localStorage.setItem('adminpw',pw);location.href='/admin/dashboard';}else document.getElementById('err').style.display='block';});
}
...
<script>
const pw=localStorage.getItem('adminpw')||prompt('Password:');
